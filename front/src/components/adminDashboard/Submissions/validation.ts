...
export function validateSuggestionProcessingForm(
  ...
  if (!isOptionalUrl(form.urlSource)) {
    errors.push("L’URL source n’est pas valide.");
  }
  ...
  if (form.artistMode === "existing") {
    ...
    if (
      ...
      !isOptionalUrl(form.newArtist.imageArtists)
    ) {
      errors.push("L’image du nouvel artiste principal doit être une URL valide.");
    }
  }
  ...
  if (form.albumMode === "existing") {
    ...
    if (
      ...
      !isOptionalUrl(form.newAlbum.imageAlbum)
    ) {
      errors.push("L’image du nouvel album doit être une URL valide.");
    }
  }
    ...
    form.featurings.forEach((featuring, index) => {
      ...
      if (featuring.mode === "existing") {
        if (!featuring.existingArtistId) {
          errors.push(`Tu dois sélectionner un artiste existant pour le featuring #${position}.`);
        }
      } else {
        ...
        if (
          ...
          !isOptionalUrl(featuring.newArtist.imageArtists)
        ) {
          errors.push(`L’image du featuring #${position} doit être une URL valide.`);
        }
      }
    });
  }
